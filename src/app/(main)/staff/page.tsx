...
export default function StaffPage() {
  ...
  const calculateSalary = (member: typeof staff[0]) => {
    if (member.position.type === 'monthly') {
      return member.salary;
    }
    if (member.position.type === 'hourly' && member.hoursWorked) {
      const totalHours = Object.values(member.hoursWorked).reduce((sum, h) => sum + h, 0);
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Calculated Salary (Monthly)</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {staff.map((member) => (
              <TableRow key={member.id}>
                ...
                <TableCell className="text-right">${calculateSalary(member)?.toLocaleString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
